...
class BinanceFutures:
    def __init__(self, api_key, api_secret, symbol='btcusdt', testnet=False, orderIDPrefix='bot_bf_', postOnly=False, timeout=999999999999):
        ...
        # my new instance variables
        self.mid_price_list = []
        self.mid_price_change_list = []
        self.market_arrival_depth_list = []
        self.A = 0
        self.k = 0
        self.volatility = 0
        self.time_snap = 0
    ...
    async def __on_message(self, message):
        ...
        if evt == 'listenKeyExpired':
            ...
            exch_timestamp = int(transaction_time)
            self.time_snap = exch_timestamp

            side = -1 if data['m'] else 1

            market_depth = self.depth
            ...
            # Calculate mid_price_change and market_arrival_depth
            if len(self.mid_price_list) == 0:
                self.mid_price_list.append(mid)
            else:
                mid_price_change = mid - self.mid_price_list[-1]
                depth = -np.inf

                if side == 1:
                    market_order_arrival_depth = np.nanmax([float(price) - mid, depth])
                else:
                    market_order_arrival_depth = np.nanmax([mid - float(price), depth])
                self.mid_price_list.append(mid)
                
                # Save mid_price_change and market_arrival_depth to lists
                self.mid_price_change_list.append([exch_timestamp, mid_price_change])
    ...
    def trim_data_lists(self):
        ...
        while self.mid_price_change_list and self.mid_price_change_list[0][0] < ten_minutes_ago:
            self.mid_price_change_list.pop(0)
    ...
    def calculate_intensity_and_volatility(self):
        ...
        five_seconds_ago = current_time - 5_000
        tmp = np.zeros(500, np.float64)
        ticks = np.arange(len(tmp)) + .5 # numpy array for saving the distance of market orders from the mid price

        # Updates A, k, and the volatility every 5-sec.
        if self.record_timestamp <= five_seconds_ago:
            self.record_timestamp = time.time()
            # Window size is 10-minute.
            for i in range(len(self.market_arrival_depth_list) - 1):
                if current_time - 600_000 >= self.market_arrival_depth_list[i][0]:
                    # Calibrates A, k
                    tmp[:] = 0
                    second_elements_depth = [sublist[1] for sublist in self.market_arrival_depth_list[i:]]
                    second_elements_price_change = [sublist[1] for sublist in self.mid_price_change_list[i:]]
                    lambda_ = self.measure_trading_intensity(second_elements_depth, tmp)
                    lambda_ = lambda_[:30] / 600
                    self.oder_counted_list = lambda_
                    x = ticks[:len(lambda_)]
                    y = np.log(lambda_)
                    k_, logA = self.linear_regression(x, y)
                    self.A = np.exp(logA)
                    self.k = -k_
            
                    # Updates the volatility.
                    self.volatility = np.nanstd(second_elements_price_change) * np.sqrt(10)

    def measure_trading_intensity(self, order_arrival_depth, out):
        max_tick = 0
    ...
    async def __curl_binancefutures(self, path, query=None, timeout=None, verb=None, rethrow_errors=True, max_retries=None):
        ...
        if verb == 'DELETE':
            try:
                temp = ''
                for item in query['origClientOrderIdList']:
                    temp.join(item)
                print(temp)
            except:
                pass
